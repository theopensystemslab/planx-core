...
describe("ContactInput details are set correctly based on the breadcrumbs", () => {
  ...
  it("orders contact parts correctly when all fields are present", () => {
    ...
    const contactInputItem = proposalDetails?.filter((detail) => detail.question === "Your contact details")?.[0];

    expect(contactInputItem?.responses[0]?.["value"]).toEqual("Ms Jane Doe Open Systems Lab 0123456789 test@test.com");
  });
  ...
  it("orders contact parts correctly when only required fields are present", () => {
    ...
    expect(contactInputItem?.responses[0]?.["value"]).toEqual("Jane Doe 0123456789 test@test.com");
  });
});
