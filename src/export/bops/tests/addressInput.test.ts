...
describe("AddressInput details are set correctly based on the breadcrumbs", () => {
  ...
  it("orders address parts correctly when all fields are present", () => {
    ...
    const addressInputItem = proposalDetails?.filter((detail) => detail.question === "What's your address")?.[0];

    expect(addressInputItem?.responses[0]?.["value"]).toEqual("10 Main Street, Apt 1, Southwark, London, SE5 0HU, UK");
  });
  ...
  it("orders address parts correctly when only required fields are present", () => {
    ...
    expect(addressInputItem?.responses[0]?.["value"]).toEqual("10 Main Street, Southwark, SE5 0HU");
  });
});
